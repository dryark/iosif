...
void runXtest( void *device );
void run_wda( ucmd *cmd ) { g_cmd = cmd; waitForConnect( runWda ); }
void run_xtest( ucmd *cmd ) { g_cmd = cmd; waitForConnect( runXtest ); }
...
void runWda( void *device ) {
  ...
  devUp( device );
  CFDictionaryRef runnerInfo = getAppInfo( device, "com.appium.WebDriverAgentRunner.xctrunner" );
  devDown( device );
  ...
  char *runnerBundleId = "com.appium.WebDriverAgentRunner.xctrunner";
...
uint64_t launchTestRunner(
  ...
  channelT *processChan
	...
	tDICT *environment = tDICT__newPairs( 22,
